...
Function `join` is used to join two different statements in the same period.
We used merge for merging two statements of the same type and different period.

The leading dot instructs the calculate function to hide the values. In our case
...
```{r graph1, fig.width=7.3, fig.height=3.5}
...
```{r graph2, fig.width=7.3, fig.height=3.5}
...
```{r graph3, fig.width=7.3, fig.height=3.5}
